...
maxs=data.frame("votes"=data[[6]],"town"=data[[10]],"party"=data[[8]])
...
for(i in 1:368){
  for(j in 1:368){
    if(newdata$Category[i]==data[[10]][j]){
      test$townname[i]=newdata$Category[i]
      test$ref[i]=newdata$x[i]
      test$countryname[i]=data[[9]][j]
      test$vot[i]=data[[7]][j]
      test$party[i]=data[[8]][j]
      break
    }
